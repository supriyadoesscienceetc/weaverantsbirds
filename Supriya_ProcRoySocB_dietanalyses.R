...
tiff("FigS2.tiff",width = 1200, height = 1600, units = "px", res= 300)
...
library(ggpubr)
tiff("exclusiondiet3.tiff",width = 1200, height = 1600, units = "px", res= 300)
...
ordihull(example_NMDS2,groups=nmdsdat4$Type,draw="polygon",col="grey90", label=T)
...
ordihull(fam_NMDS,groups=nmdsfam3$Type,draw="polygon",col="grey90", label=T)

#genus level
nmdsgenus<- read.csv("nmdsgenus.csv")
nmdsgenus3<- nmdsgenus[1:57,]
nmdsgenus2<- nmdsgenus3[,-c(1,2)]
genus_NMDS=metaMDS(nmdsgenus2,k=2, trymax=1000) 
...
species_NMDS=metaMDS(nmdsspecies2,k=2, trymax=1000) 
